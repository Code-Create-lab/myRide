...
class AdminController extends Controller
{
    ...
    public function rideReport(Request $request)
    {
        ...
        $plusTransactions   = Ride::where('driver_id', '!=', 0)
            ->whereDate('created_at', '>=', $starDate)
            ->whereDate('created_at', '<=', $endDate)
            ->selectRaw('DATE_FORMAT(created_at, "' . $timePeriod->sql_date_format . '") as date,count(*) as count')
            ->orderBy('date', 'asc')
            ->groupBy('date')
            ->get();
        ...
        for ($i = 0; $i < $timePeriod->take; $i++) {
            ...
            $plusAmount  = $plusTransaction ? $plusTransaction->count : 0;
            // $minusAmount = $minusTransaction ? $minusTransaction->amount : 0;


            // dd($date,$timePeriod->php_date_format);
            $data[$date] = [
                'plus_amount'  => $plusAmount,
                // 'minus_amount' => $minusAmount
            ];
