...
class CouponController extends Controller
{
    ...
    public function applyCoupon(Request $request, $id)
    {
        ...
        // $ride  = Ride::where('id', $id)->where('user_id', auth()->id())->orWhere('status', Status::RIDE_END)->orWhere('status', 5)->orWhere('status', 0)->first();
      
        $ride = Ride::where(function ($query) use ($id) {
            $query->where('id', $id)
                  ->where('user_id', auth()->id());
        })
        ->where(function ($query) {
            $query->where('status', Status::RIDE_END)
                  ->orWhere('status', 5)
                  ->orWhere('status', 0);
        })
        ->first();
        ...
        $couponUsingTime = Ride::where('applied_coupon_id', $coupon->id)->where('user_id', $ride->user_id)->whereIn('status', [Status::RIDE_COMPLETED, Status::RIDE_END, 5,0])->count();


        // dd($couponUsingTime, $coupon->maximum_using_time);
    ...
    public function removeCoupon($id)
    {
        ...
        $notify[] =  'Coupon remove successfully';
