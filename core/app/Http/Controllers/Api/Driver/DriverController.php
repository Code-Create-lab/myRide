...
class DriverController extends Controller
{
    public function dashboard()
    {
        ...
        if($getDriver){
            ...
            if ($driver->online_status  == Status::YES && $driver->dv == Status::VERIFIED && $driver->vv == Status::VERIFIED) {
                ...
                //       ->whereNotIn('status', [Status::BID_CANCELED, Status::BID_REJECTED]);
                // })
                ->whereBetween('pickup_date_time', [Carbon::now()->subMinutes(60), Carbon::now()->addMinutes(60)]) // ✅ Condition Updated
                ->with('user', 'service')
                ->latest('id')
                ->paginate(getPaginate());
                // dd(  $rides, Carbon::now()->subMinutes(60) );
                // dd($rides ,$driver->zone_id , @$driver->service_id);
        } else {
            $rides = null;
        }
    ...
    public function vehicleVerification()
    {
        ...
        $getVehicleLists = Vehicle::where('is_occupied', 0)->with("service")->active()->get();

        return apiResponse("vehicle_form", "success", $notify, [
            'form'               => $form->form_data,
            'vehicles'           => $getVehicleLists,
    ...
    public function mapVechicleByDriver(Request $request){
        ...
        // dd($vehicle->service_id);
        $driver->vv            = Status::VERIFIED;
        $driver->service_id   = $vehicle->service_id;
        $driver->online_status   = 1;
        $driver->save();

        // dd($vehicle,$request->vehicle_Id);
        $vehicle->is_occupied = 1;
        $vehicle->save();

        $vehicleSession = new VehicleSession();

        $vehicleSession->driver_id = $request->driver_id;
        $vehicleSession->vehicle_Id = $request->vehicle_Id;
        $vehicleSession->login_in_time = Carbon::now();
        $vehicleSession->save();
    ...
    public function vehicleVerificationStore(Request $request)
    {
        ...
        $driver->brand_id      = $request->brand_id ?? 0;
    ...
    public function addDeviceToken(Request $request)
    {
        ...
        $deviceToken = DeviceToken::where('token', $request->token)->where('driver_id',auth()->user()->id)->first();
