...
class Driver extends Authenticatable
{
    ...
    //     static::addGlobalScope('service', function ($query) {
    //         $query->with('vehicle_session');
    //     });
    // }
    ...
    public function vehicleData(): Attribute
    {
        return new Attribute(function () {
            return $this->vehicleSession();
        });
    }

    public function vehicleSession()
    {
        $getVehicle =  VehicleSession::where('driver_id', $this->id ?? [])->where('login_out_time' , null)->with(['vehicle' => function ($query) {
            $query->select('id', 'model as name', 'number');
        }])->first()?->vehicle;
        
        // $vhicleData =  $getVehicle->vehicle;
...
// dd($getVehicle);
        $arraydata = [
                [
                    'name' => 'Name',
                    "type" => "text",
                    "value" => $getVehicle?->name
                ],
                [
                    'name' => 'Number',
                    "type" => "text",
                    "value" => $getVehicle?->number
                ], 
            ];
        // dd($vhicleData);
        return    $arraydata;

        // return $this->hasOne(VehicleSession::class, 'driver_id');
    }
    ...
    {
        return $query->where('status', Status::USER_ACTIVE)->where('vv', Status::VERIFIED);
    }
